...
export const zhCN = {
  ...
  style: {
    ...
    pack: {
      ...
      runtimeScreenContextTitle: '屏幕上下文段',
      runtimeScreenContextDesc: '来自当前屏幕 OCR',
      runtimeScreenContextEmpty: '当前不会附加',
  ...
  settings: {
    ...
    ocr: {
      title: '屏幕上下文 (OCR)',
      enableLabel: '启用屏幕上下文',
      enableHint: '每次听写开始时自动截取当前屏幕并识别文字，作为润色上下文注入系统提示词。',
      providerLabel: 'OCR 提供方',
      providerHint: '选择本地 OCR 引擎。RapidOCR 使用 ONNX 模型，中文效果更好；Windows 11/10 自带 WinRT OCR 无需模型但中文支持依赖系统语言包。',
      modelsBaseDirLabel: '模型存储目录',
      downloadMirrorLabel: '下载镜像前缀',
      downloadMirrorHint: '留空使用 GitHub 官方地址；自建镜像请填到 release 目录前缀（应用会自动追加 /模型文件名.onnx），例如 https://mirror.example.com/rapidocr/v2.0.0',
      downloadModel: '下载模型',
      cancelDownload: '取消下载',
      deleteModel: '删除模型',
      downloadCompleted: '模型已下载完成',
      testRecognition: '测试当前屏幕识别',
      testResult: '识别结果',
      noTextRecognized: '未识别到文字',
      screenRecordingDenied: 'macOS 屏幕录制权限未开启，请前往系统设置 → 隐私与安全性 → 屏幕录制，勾选 OpenLess 后重启应用。',
      winrtLanguageMissing: 'WinRT OCR 无法创建中文识别引擎。请安装中文 OCR 语言包：设置 → 时间和语言 → 语言和区域 → 中文 → 语言选项 → 可选功能 → 光学字符识别(OCR)。',
      winrtLanguagePackHint: '需安装系统中文 OCR 语言包',
      platforms: '支持平台',
      state: {
        notStarted: '未开始',
        inProgress: '下载中',
