...
export const zhTW: typeof zhCN = {
  ...
  style: {
    ...
    pack: {
      ...
      runtimeScreenContextTitle: '螢幕上下文段',
      runtimeScreenContextDesc: '來自目前螢幕 OCR',
      runtimeScreenContextEmpty: '目前不會附加',
  ...
  settings: {
    ...
    ocr: {
      title: '螢幕上下文 (OCR)',
      enableLabel: '啟用螢幕上下文',
      enableHint: '每次聽寫開始時自動截取目前畫面並辨識文字，作為潤色上下文注入系統提示詞。',
      providerLabel: 'OCR 提供方',
      providerHint: '選擇本地 OCR 引擎。RapidOCR 使用 ONNX 模型，中文效果較好；WinRT OCR 內建於 Windows 10/11，但中文需安裝語言套件。',
      modelsBaseDirLabel: '模型儲存目錄',
      downloadMirrorLabel: '下載鏡像前綴',
      downloadMirrorHint: '留空使用 GitHub 官方網址；自建鏡像請填到 release 目錄前綴（應用會自動追加 /模型檔名.onnx），例如 https://mirror.example.com/rapidocr/v2.0.0',
      downloadModel: '下載模型',
      cancelDownload: '取消下載',
      deleteModel: '刪除模型',
      downloadCompleted: '模型已下載完成',
      testRecognition: '測試目前畫面辨識',
      testResult: '辨識結果',
      noTextRecognized: '未辨識到文字',
      screenRecordingDenied: 'macOS 螢幕錄製權限未開啟，請前往系統設定 → 隱私權與安全性 → 螢幕錄製，勾選 OpenLess 後重新啟動應用程式。',
      winrtLanguageMissing: 'WinRT OCR 無法建立中文辨識引擎。請安裝中文 OCR 語言套件：設定 → 時間與語言 → 語言與地區 → 中文 → 語言選項 → 選用功能 → 光學字元辨識(OCR)。',
      winrtLanguagePackHint: '需安裝系統中文 OCR 語言套件',
      platforms: '支援平台',
      state: {
        notStarted: '未開始',
        inProgress: '下載中',
