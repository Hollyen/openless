...
export const ko: typeof zhCN = {
  ...
  style: {
    ...
    pack: {
      ...
      runtimeScreenContextTitle: '화면 컨텍스트 블록',
      runtimeScreenContextDesc: '현재 화면 OCR에서',
      runtimeScreenContextEmpty: '현재 미리보기에는 추가되지 않습니다.',
  ...
  settings: {
    ...
    ocr: {
      title: '화면 컨텍스트 (OCR)',
      enableLabel: '화면 컨텍스트 활성화',
      enableHint: '음성 입력 시작 시 현재 화면을 캡처하고 인식된 텍스트를 다듬기 문맥으로 주입합니다.',
      providerLabel: 'OCR 제공처',
      providerHint: '로컬 OCR 엔진을 선택하세요. RapidOCR은 ONNX 모델을 사용해 중국어 인식이 우수하고, WinRT OCR은 Windows 10/11에 내장되어 있으나 중국어는 언어 팩이 필요합니다.',
      modelsBaseDirLabel: '모델 저장 디렉터리',
      downloadMirrorLabel: '다운로드 미러 접두사',
      downloadMirrorHint: '비워 두면 GitHub 공식 URL을 사용합니다. 미러를 사용할 경우 릴리스 디렉터리 접두사까지 입력하세요(앱이 자동으로 /모델 파일명.onnx를 추가합니다). 예: https://mirror.example.com/rapidocr/v2.0.0',
      downloadModel: '모델 다운로드',
      cancelDownload: '다운로드 취소',
      deleteModel: '모델 삭제',
      downloadCompleted: '모델 다운로드 완료',
      testRecognition: '현재 화면 인식 테스트',
      testResult: '인식 결과',
      noTextRecognized: '인식된 텍스트 없음',
      screenRecordingDenied: 'macOS 화면 기록 권한이 활성화되지 않았습니다. 시스템 설정 → 개인 정보 보호 및 보안 → 화면 기록에서 OpenLess를 활성화한 후 앱을 완전히 종료하고 다시 시작하세요.',
      winrtLanguageMissing: 'WinRT OCR이 중국어 인식 엔진을 만들 수 없습니다. 중국어 OCR 언어 팩을 설치하세요: 설정 → 시간 및 언어 → 언어 및 지역 → 중국어 → 언어 옵션 → 선택적 기능 → 광학 문자 인식(OCR).',
      winrtLanguagePackHint: '시스템 중국어 OCR 언어 팩 필요',
      platforms: '지원 플랫폼',
      state: {
        notStarted: '시작 전',
        inProgress: '다운로드 중',
