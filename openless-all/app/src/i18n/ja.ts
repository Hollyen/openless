...
export const ja: typeof zhCN = {
  ...
  style: {
    ...
    pack: {
      ...
      runtimeScreenContextTitle: '画面コンテキストブロック',
      runtimeScreenContextDesc: '現在の画面の OCR から',
      runtimeScreenContextEmpty: '現在のプレビューでは付加されません。',
  ...
  settings: {
    ...
    ocr: {
      title: '画面コンテキスト (OCR)',
      enableLabel: '画面コンテキストを有効化',
      enableHint: '音声入力開始時に現在の画面をキャプチャし、認識したテキストを補正の文脈として注入します。',
      providerLabel: 'OCR プロバイダー',
      providerHint: 'ローカル OCR エンジンを選択します。RapidOCR は ONNX モデルで中国語の精度が高く、WinRT OCR は Windows 10/11 に内蔵されていますが、中国語には言語パックが必要です。',
      modelsBaseDirLabel: 'モデル保存ディレクトリ',
      downloadMirrorLabel: 'ダウンロードミラー接頭辞',
      downloadMirrorHint: '空欄の場合は GitHub 公式 URL を使用します。ミラーを使用する場合はリリースディレクトリの接頭辞まで入力してください（アプリが自動的に /モデルファイル名.onnx を追加します）。例: https://mirror.example.com/rapidocr/v2.0.0',
      downloadModel: 'モデルをダウンロード',
      cancelDownload: 'ダウンロードをキャンセル',
      deleteModel: 'モデルを削除',
      downloadCompleted: 'モデルのダウンロードが完了しました',
      testRecognition: '現在の画面をテスト',
      testResult: '認識結果',
      noTextRecognized: 'テキストが認識されませんでした',
      screenRecordingDenied: 'macOS の画面収録権限が有効になっていません。システム設定 → プライバシーとセキュリティ → 画面収録で OpenLess を有効にし、アプリを完全に終了して再起動してください。',
      winrtLanguageMissing: 'WinRT OCR で中国語の認識エンジンを作成できません。中国語 OCR 言語パックをインストールしてください：設定 → 時刻と言語 → 言語と地域 → 中国語 → 言語オプション → オプション機能 → 光学式文字認識 (OCR)。',
      winrtLanguagePackHint: 'システムの中国語 OCR 言語パックが必要です',
